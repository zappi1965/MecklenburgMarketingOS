...
export default function Home(){
  ...
  const customerViews=(activeCustomer?.revenue===0 && activeCustomer?.rating===0)?['salesPage']:['customerDashboard','booking','seo','reviewAI','customerTickets','templates','invoices','competitorAds','locations','goals','reminders','integrations']
  ...
  return <div data-theme={theme} data-design={design} data-color={colorScheme} className="page fadeDesign"><div className="app"><aside className="side"><h2>MM OS</h2><p><b>{profile.full_name}</b><br/><span>{profile.role}</span>{activeCustomer&&<><br/><span>{customerEnv?'Kundenumgebung: ':'Aktiv: '}{activeCustomer.name}</span></>}</p>{profile.role!=='customer'&&customerEnv&&<button className="btn" onClick={()=>{setCustomerEnv(false);setView('dashboard')}}>⬅ Adminbereich</button>}{views.map(v=><button key={v} className={view===v?'btn':'btn secondary'} onClick={()=>setView(v)}>{viewLabel(v)}</button>)}<button className="btn secondary" onClick={()=>setTheme(theme==='dark'?'light':'dark')}>{theme==='dark'?'☀️ Light':'🌙 Dark'}</button><button className="btn red" onClick={logout}>🚪 Logout</button></aside><main className="main"><div className="top"><div><h1>{viewLabel(view)}</h1>{activeCustomer&&<p className="muted">{customerEnv?'Kundenumgebung geöffnet':'Adminbereich'} · {activeCustomer.name}</p>}</div></div>
    ...
    {view==='leadMeetings'&&<LeadMeetingsSafe customers={customers} openCrm={openCrm}/>} 
    {view==='tickets'&&<AdminTickets openCrm={openCrm}/>}
    {view==='templates'&&<Templates/>}
    {view==='invoices'&&<InvoicesSafe customer={activeCustomer} admin={!customerEnv} customers={customers}/>} 
    {view==='leadSearches'&&<LeadSearchesSafe/>} 
    {view==='qr'&&<QRCampaignsSafe customers={customers} openCrm={openCrm}/>} 
    ...
    {view==='reminders'&&<RemindersSafe customer={activeCustomer} admin={!customerEnv} customers={customers}/>}  
...
/* v12.2 Reference Safety Components */
function SafeCard({title,children}){return <div className="card"><h2>{title}</h2>{children}</div>}

function LeadMeetingsSafe({customers=[],openCrm=()=>{}}){
  const [items,setItems]=useState([]),[open,setOpen]=useState(false),[form,setForm]=useState({customer_id:'',meeting_date:today(),start_time:'10:00',end_time:'11:00',goal:''})
  useEffect(()=>{load()},[])
...
function InvoicesSafe({customer,admin=false,customers=[]}){
  ...
  async function add(){const cid=form.customer_id||customer?.id;if(!cid)return alert('Bitte Kunde wählen.');await supabase.from('invoices').insert({invoice_number:'RE-'+Date.now(),customer_id:cid,amount:Number(form.amount||0),service:form.service,due_date:plus14(),status:'Offen'});setForm({customer_id:customer?.id||'',amount:0,service:''});load()}
  async function status(id,s){await supabase.from('invoices').update({status:s}).eq('id',id);load()}
  return <div className="grid"><SafeCard title="Rechnung erstellen">{admin&&<select className="input" value={form.customer_id} onChange={e=>setForm({...form,customer_id:e.target.value})}><option value="">Kunde wählen</option>{customers.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select>}<input className="input" type="number" placeholder="Betrag" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})}/><input className="input" placeholder="Leistung" value={form.service} onChange={e=>setForm({...form,service:e.target.value})}/><button className="btn" onClick={add}>Speichern</button></SafeCard><SafeCard title="Rechnungen"><table className="table"><tbody>{items.map(i=><tr key={i.id}><td>{i.invoice_number}</td><td>{i.customers?.name}</td><td>{money(i.amount)}</td><td><select className="input" value={i.status} onChange={e=>status(i.id,e.target.value)}><option>Offen</option><option>Bezahlt</option><option>Überfällig</option></select></td></tr>)}</tbody></table></SafeCard></div>
}

function LeadSearchesSafe(){
  const[items,setItems]=useState([]),[form,setForm]=useState({name:'',branch:'',area:''})
  useEffect(()=>{load()},[])
  async function load(){const{data}=await supabase.from('lead_searches').select('*').order('created_at',{ascending:false});setItems(data||[])}
  async function add(){await supabase.from('lead_searches').insert({name:(form.name+' '+form.area).trim()||'Neue Suche',branch:form.branch,area:form.area,status:'Aktiv'});setForm({name:'',branch:'',area:''});load()}
  return <div className="grid"><SafeCard title="Suche hinzufügen"><input className="input" placeholder="Suchname" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/><input className="input" placeholder="Branche" value={form.branch} onChange={e=>setForm({...form,branch:e.target.value})}/><input className="input" placeholder="Ort/Gebiet" value={form.area} onChange={e=>setForm({...form,area:e.target.value})}/><button className="btn" onClick={add}>Speichern</button></SafeCard><SafeCard title="Suchen">{items.map(i=><div className="item" key={i.id}>{i.name}<br/>{i.branch} · {i.area}<br/><span className="badge">{i.status}</span></div>)}</SafeCard></div>
}

function QRCampaignsSafe({customers=[],openCrm=()=>{}}){
  const[items,setItems]=useState([]),[customer,setCustomer]=useState(''),[mail,setMail]=useState(''),[preview,setPreview]=useState(null)
  useEffect(()=>{load()},[])
  async function load(){const{data}=await supabase.from('qr_campaigns').select('*, customers(*)').order('created_at',{ascending:false});setItems(data||[])}
  async function add(){const c=customers.find(x=>x.id===customer);if(!c)return alert('Bitte Kunde wählen.');await supabase.from('qr_campaigns').insert({customer_id:customer,name:c.name,target_url:'/review/'+customer,negative_feedback_email:mail||c.email});setCustomer('');setMail('');load()}
  return <div className="grid"><SafeCard title="QR Kampagne"><select className="input" value={customer} onChange={e=>setCustomer(e.target.value)}><option value="">Kunde wählen</option>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><input className="input" placeholder="Feedback Mail" value={mail} onChange={e=>setMail(e.target.value)}/><button className="btn" onClick={add}>Speichern</button></SafeCard><SafeCard title="Kampagnen">{items.map(i=><div className="item" key={i.id} onClick={()=>setPreview(i)}>{i.name}<br/>{i.customers?.name}<br/>{i.negative_feedback_email}</div>)}</SafeCard>{preview&&<Modal title={'QR Preview · '+preview.name} onClose={()=>setPreview(null)}><CustomerChip customer={preview.customers} onOpen={openCrm}/><div style={{width:170,height:170,background:'repeating-linear-gradient(45deg,#111 0 8px,#fff 8px 16px)',border:'10px solid white',borderRadius:18,margin:'20px auto'}}></div><p>Ziel: {preview.target_url}</p></Modal>}</div>
}

function RemindersSafe({customer,admin=false,customers=[]}){
  const [items,setItems]=useState([])
  useEffect(()=>{load()},[customer])
  async function load(){let q=supabase.from('reminders').select('*, invoices(*), customers(*)').order('created_at',{ascending:false});if(customer&&!admin)q=q.eq('customer_id',customer.id);const{data}=await q;setItems(data||[])}
  return <SafeCard title="Mahnungen">{items.length===0&&<p className="muted">Noch keine Mahnungen.</p>}{items.map(m=><div className="item" key={m.id}>{m.reminder_number||'Mahnung'} · {m.level}<br/>{m.customers?.name}<br/>Gebühr: {money(m.fee)}</div>)}</SafeCard>
}
