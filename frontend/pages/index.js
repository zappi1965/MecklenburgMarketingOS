...
async function unlockTools(customerId, packageName, customTools=[]){const cfg=PACKAGE_CATALOG[packageName]||PACKAGE_CATALOG.Individuell;const tools=packageName==='Individuell'?customTools:cfg.tools;if(!customerId||!tools.length)return;for(const tool_key of tools){await supabase.from('customer_tool_access').upsert({customer_id:customerId,tool_key,enabled:true,source_package:packageName},{onConflict:'customer_id,tool_key'})}}
...
export default function Home(){
  ...
  const adminViews=['dashboard','crm','customerDemo','users','customers','leadMeetings','tickets','templates','invoices','reminders','leadSearches','qr','packages','pipeline','goalsAdmin','growthTools','referralsAdmin','notifications','timeline','automations','search','smart','roles','audit']
  const baseCustomerViews=(activeCustomer?.revenue===0 && activeCustomer?.rating===0)?['salesPage']:['customerDashboard']; const [enabledViews,setEnabledViews]=useState([]); useEffect(()=>{if(activeCustomer)getEnabledToolViews(activeCustomer.id).then(setEnabledViews)},[activeCustomer]); const customerViews=[...new Set([...baseCustomerViews,...enabledViews])]
