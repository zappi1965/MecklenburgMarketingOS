...
export default function Home(){
  ...
  const adminViews=['dashboard','crm','customerDemo','users','customers','leadMeetings','tickets','templates','invoices','reminders','leadSearches','qr','audit']
  const customerViews=(activeCustomer?.revenue===0 && activeCustomer?.rating===0)?['salesPage']:['customerDashboard','booking','seo','customerTickets','templates','invoices','reminders','integrations']
...
function SalesPage({customer}){
  ...
  async function requestPackage(packageName){
    const {error}=await supabase.from('package_requests').insert({customer_id:customer.id,package_name:packageName,status:'Neu'})
    if(error)return alert(error.message)
    setSent(packageName)
    alert('Paket angefragt: '+packageName+'. Wir melden uns zeitnah.')
  }
  ...
  return <div>
    ...
    <div className="grid">
      <div className="card"><h2>Starter</h2><div className="kpi">199€</div><p>Review-System, QR Kampagne, einfache Reports.</p><button className="btn" onClick={()=>requestPackage('Starter')}>Starter anfragen</button></div>
      <div className="card"><h2>Growth</h2><div className="kpi">499€</div><p>SEO Dashboard, Booking, Reviews, monatlicher Report.</p><button className="btn" onClick={()=>requestPackage('Growth')}>Growth anfragen</button></div>
      <div className="card"><h2>Premium</h2><div className="kpi">899€</div><p>Vollständiges Kundenportal, Automationen, Reporting, Support.</p><button className="btn" onClick={()=>requestPackage('Premium')}>Premium anfragen</button></div>
    </div>
    <div className="grid">
...
function CRMOffers({customer}){
  ...
  const [form,setForm]=useState({title:'',amount:0,status:'Offen',package_name:'Starter'})
  useEffect(()=>{if(customer)load()},[customer])
  async function load(){
  ...
  async function add(){
    ...
    setForm({title:'',amount:0,status:'Offen',package_name:'Starter'})
    setFile(null)
    load()
  }
  return <div className="grid"><div className="card"><h2>Angebot hochladen/erstellen</h2>
    <select className="input" value={form.package_name} onChange={e=>setForm({...form,package_name:e.target.value})}><option>Starter</option><option>Growth</option><option>Premium</option><option>Individuell</option></select>
    <input className="input" placeholder="Leistung / Titel" value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/>
    <input className="input" type="number" placeholder="Preis / Monatsumsatz" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})}/>
