...
export default function BenchmarkAdmin() {
  ...
  async function loadCustomerScope(preferred?: string | null) {
    const scope = await resolveCustomerScope(preferred || customerId)
    setCustomers(scope.customers)
    if (scope.customerId) setCustomerId(scope.customerId)
  }

  useEffect(() => { void loadTargets(branch) }, [branch])
  useEffect(() => { void loadCustomerScope() }, [])
  ...
  async function generateForCustomer() {
    ...
    setBusy(true); setMsg(null)
    try {
      const res = await brancheBenchmarkClient.generate(customerId, period)
      setMsg(`Benchmark-Report erzeugt${res.report?.pdf_url ? ' und PDF gespeichert.' : '.'}`)
    } catch (e: any) { setMsg(e?.message || 'Report-Erzeugung fehlgeschlagen.') } finally { setBusy(false) }
  }
        ...
        <section className="adminCard" style={{ display: 'grid', gap: 10, maxWidth: 520 }}>
          ...
          {customers.length ? (
            <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
              <label><span>Kunde für Report</span><select className="input" value={customerId || ''} onChange={(e) => setCustomerId(e.target.value)}>{customers.map((c) => <option key={c.id} value={c.id}>{customerOptionLabel(c)}</option>)}</select></label>
              <button className="btn ghost" onClick={generateForCustomer} disabled={busy}>Report für ausgewählten Kunden erzeugen</button>
            </div>
          ) : null}
        </section>
