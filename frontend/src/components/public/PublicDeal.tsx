...
export default function PublicDeal({ slug, initialDeal, initialEnded }: { slug: string; initialDeal?: any; initialEnded?: boolean }) {
  const [deal, setDeal] = useState<any>(initialDeal || null)
  const [state, setState] = useState<'loading' | 'active' | 'ended' | 'notfound'>(
    initialEnded ? 'ended' : initialDeal ? 'active' : 'loading'
  )
  const countdown = useCountdown(deal?.expires_at)

  useEffect(() => {
    if (initialDeal || initialEnded) {
      if (initialDeal) v33FunctionalClient.publicDealTrack(slug, 'view').catch(() => {})
      return
    }
    async function load() {
      try {
