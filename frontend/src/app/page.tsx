...
function QRCodes({store,cid,setCid,role='admin',activeView='qr'}:any){
 ...
 if(section!=='campaigns'){
  return <><Head title="QR Kampagnen verwalten" sub={`Bereich: ${sectionTitle} · alle QR-, Zielseiten-, Punkte- und Code-Einstellungen an einem Ort.`}/><QrSectionTabs/><QrKpiCards/>{role==='admin'&&<CentralCustomerSelector store={store} cid={activeCustomer} setCid={(id:string)=>{setCustomer(id);setCid?.(id)}} title="Kunde für QR-Konfiguration auswählen" sub="Alle QR-Bereiche beziehen sich auf diesen Kunden."/>}<V40ErrorBoundary moduleName={section}><V30ToolModule view={section} store={store} cid={activeCustomer} role={role} setCid={(id:string)=>{setCustomer(id);setCid?.(id)}}/></V40ErrorBoundary></>
 }
 if(section==='public_landing')return <><Head title="QR Zielseite" sub="Texte, Punkte, Limits, Prämien und finale Regeln werden hier pro Kampagne gespeichert."/><QrSectionTabs/><QrKpiCards/><V42PublicLandingManager store={store} cid={activeCustomer}/></>
 if(section==='staff_codes')return <><Head title="Mitarbeitercodes" sub="PINs zur Bestätigung vor Ort. Punkte und Prämien bleiben unter QR Zielseite."/><QrSectionTabs/><QrKpiCards/><V30ToolModule view="staff_codes" store={store} cid={activeCustomer} role={role} setCid={setCid}/></>
