...
function demoCustomers(d:any){
 ...
 ;[...(d.demo_customers||[]),...(d.customers||[]).filter((c:any)=>isDemoRecord(c))].filter((c:any)=>c&&c.id).forEach((c:any)=>byId.set(c.id,c))
...
function Search({items,value,onChange,placeholder}:any){const [q,setQ]=useState('');const safeItems=(items||[]).filter((x:any)=>x&&x.id);const s=safeItems.find((x:any)=>x.id===value);const list=safeItems.filter((x:any)=>((x.name||'')+(x.branch||'')+(x.email||'')).toLowerCase().includes(q.toLowerCase()));return <div style={{position:'relative'}}><input className="input" placeholder={placeholder} value={s&&!q?(s.name||''):q} onChange={e=>{setQ(e.target.value);if(s)onChange('')}}/>{q&&<div className="card floating">{list.map((x:any)=><button className="nav" key={x.id} onClick={()=>{onChange(x.id);setQ('')}}>{x.name||'(ohne Name)'}<div className="sub">{x.branch} · {x.package_name}</div></button>)}</div>}</div>}
