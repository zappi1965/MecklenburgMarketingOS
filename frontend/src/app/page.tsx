...
function mainLandingSettings(d:any){const saved=((d.landing_page_settings||[]).find((x:any)=>x.id==='main'||x.scope==='public_home')||{});return {...defaultMainLandingSettings,...saved,packages:{...defaultMainLandingSettings.packages,...(saved.packages||{})}}}
...
function Integrations({store,cid,role}:any){
 ...
 async function save(){
  ...
  await store.create('integrations',{customer_id:cid,name:f.name,provider,api_key:f.api_key,site_url:f.site_url,property_id:f.property_id,status:f.api_key?'API-Key hinterlegt':'OAuth/API vorbereiten',seo_enabled:true})
  setMsg('Integration gespeichert. SEO-Daten können über Sync aktualisiert werden.')
 }
 async function oauth(){
  try{const r=await startGoogleAuth(cid);setMsg(r?.data?.url?'Google OAuth Fenster geöffnet.':'Google OAuth ist backendseitig noch nicht konfiguriert.')}catch(e:any){setMsg(e.message||'Google OAuth nicht erreichbar')}
 }
...
function CustomerSEO({store,cid}:any){
