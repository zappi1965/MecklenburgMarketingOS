...
const packageDefs:any={
  Starter:{
    ...
    tools:['Dashboard','Rechnungen','Tickets','Booking','Integrationen','Media Center','Pakete & Billing']
  },
  Growth:{
    price:499,
    base:'Starter',
    displayFeatures:['Alles aus Starter-Paket','SEO','Booking','Pipeline','Integrationen','Reports'],
    tools:['Dashboard','Rechnungen','Tickets','Booking','Integrationen','Media Center','Pakete & Billing','SEO Dashboard','Review Funnel','Automationen','Workflow Center']
  },
  Premium:{
    price:899,
    base:'Growth',
    displayFeatures:['Alles aus Growth-Paket','Automationen','Workflows','Rechte','Review Funnel'],
    tools:['Dashboard','Rechnungen','Tickets','Booking','Integrationen','Media Center','Pakete & Billing','SEO Dashboard','Review Funnel','Automationen','Workflow Center','Rechte & Rollen','KPI Analytics','SEO Heatmap','Client Success Score','Advanced Reports']
  }
}
...
export default function App(){
 ...
 useEffect(()=>{(async()=>{try{const profile=await getCurrentUserProfile(); if(profile){setRole(profile.role==='admin'?'admin':'customer'); if(profile.customer_id)setCid(profile.customer_id); setView('dashboard')}}finally{setLiveAuthChecked(true)}})()},[])
 ...
 const packageToolRoutes:any={'SEO Dashboard':'seo','Review Funnel':'review','Automationen':'customer_automations','Workflow Center':'customer_workflows','Rechte & Rollen':'roles','KPI Analytics':'kpi','SEO Heatmap':'heatmap','Client Success Score':'success','Advanced Reports':'advanced_reports'}
 const customerBase=['dashboard','finance','tickets','booking','integrations','media','packages']
 const customer=[...customerBase,...(packageDefs[cpkg(store.data,cid)]?.tools||[]).map((t:string)=>packageToolRoutes[t]).filter(Boolean)]
 const labels:any={dashboard:'Dashboard',crm:'CRM',finance:'Rechnungen',tickets:'Tickets',booking:'Booking',pipeline:'Pipeline',automations:'Automationen',workflows:'Workflows',activity:'Aktivitäten',media:'Media Center',qr:'QR Codes',demo_customers:'Demo Kunden',integrations:'Integrationen',packages:'Pakete & Billing',seo:'SEO Dashboard',review:'Review Funnel',customer_automations:'Automationen',customer_workflows:'Workflow Center',roles:'Rechte & Rollen',kpi:'KPI Analytics',heatmap:'SEO Heatmap',success:'Client Success Score',advanced_reports:'Advanced Reports'}
 if(role==='guest')return <div className="landing"><div className="landingNav"><div className="logo"><div className="mark">M</div>MecklenburgMarketingOS</div><div className="row"><button className="btn" onClick={()=>{window.location.href='/auth'}}>Live Login</button><button className="btn secondary" onClick={()=>{setRole('admin');setActiveAdmin('DominiqueMM')}}>Demo Admin</button><button className="btn secondary" onClick={()=>{setRole('customer');setCid(ids.barber)}}>Demo Kunde</button></div></div><section className="hero"><h1>MecklenburgMarketingOS</h1><p>Die All-in-One-Plattform für Kundenmanagement, SEO, Rechnungen, Termine, Dateien und digitale Abläufe. Kunden sehen Fortschritt, Dokumente und offene Aufgaben zentral an einem Ort – du steuerst alles sauber im Adminbereich.</p></section><div className="grid3 packageGrid">{Object.keys(packageDefs).map(p=><Card key={p} title={p}><div className="metricValue">{eur(pprice(p))}</div><div className="sub">monatlich</div><FeatureList pkg={p}/></Card>)}</div></div>
 const nav=role==='admin'?admin:customer
 return <div className={`app ${mobileNavOpen?'navOpen':''}`}><button className="mobileMenuBtn" onClick={()=>setMobileNavOpen(!mobileNavOpen)}>{mobileNavOpen?'✕':'☰'}</button><div className="mobileOverlay" onClick={()=>setMobileNavOpen(false)}></div><aside className="side"><div className="logo"><div className="mark">M</div>MMOS</div>{role==='admin'&&view!=='demo_customers'&&<Search items={store.data.customers} value={cid} onChange={setCid} placeholder="Kundensuche"/>}{nav.map(k=><button key={k} className={`nav ${view===k?'active':''}`} onClick={()=>{setView(k);setMobileNavOpen(false)}}>{labels[k]}</button>)}<button className="nav" onClick={async()=>{await supabaseAuth.auth.signOut();setRole('guest')}}>Logout</button></aside><main className="main"><div className="top"><GlobalCustomerSearch store={store} role={role} setCid={setCid} setView={setView}/><div className="topActions"><NotificationBell store={store} cid={cid} role={role} activeAdmin={activeAdmin} adminAvatars={adminAvatars}/>{role==='admin'&&<AdminToggle activeAdmin={activeAdmin} setActiveAdmin={setActiveAdmin}/>}<ProfileUpload activeAdmin={role==='admin'?activeAdmin:cname(store.data,cid)} setAdminAvatars={setAdminAvatars} adminAvatars={adminAvatars}/><Badge>{role==='admin'?activeAdmin:'Kundenportal'} · {role==='customer'?cname(store.data,cid):'Global'}</Badge></div></div><Toast m={store.toast}/>
