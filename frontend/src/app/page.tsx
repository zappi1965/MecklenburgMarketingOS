...
export default function App(){
 ...
 return <div className={`app ${mobileNavOpen?'navOpen':''}`}><button className="mobileMenuBtn" onClick={()=>setMobileNavOpen(!mobileNavOpen)}>{mobileNavOpen?'✕':'☰'}</button><div className="mobileOverlay" onClick={()=>setMobileNavOpen(false)}></div><aside className="side"><div className="logo"><div className="mark">M</div>MMOS</div>{role==='admin'&&view!=='demo_customers'&&<Search items={store.data.customers} value={cid} onChange={setCid} placeholder="Kundensuche"/>}{nav.map(k=><button key={k} className={`nav ${view===k?'active':''}`} onClick={()=>{setView(k);setMobileNavOpen(false)}}>{labels[k]}</button>)}<button className="nav" onClick={()=>{clearDemoSandbox();location.reload()}}>Demo zurücksetzen</button><button className="nav" onClick={async()=>{await supabaseAuth.auth.signOut();localStorage.removeItem('mmos_mode');setRole('guest')}}>Logout</button></aside><main className="main"><div className="top"><GlobalCustomerSearch store={store} role={role} setCid={setCid} setView={setView}/><div className="topActions"><NotificationBell store={store} cid={cid} role={role} activeAdmin={activeAdmin} adminAvatars={adminAvatars}/>{role==='admin'&&<AdminToggle activeAdmin={activeAdmin} setActiveAdmin={setActiveAdmin}/>}<ProfileUpload activeAdmin={role==='admin'?activeAdmin:cname(store.data,cid)} setAdminAvatars={setAdminAvatars} adminAvatars={adminAvatars}/><Badge>{role==='admin'?activeAdmin:'Kundenportal'} · {role==='customer'?cname(store.data,cid):'Global'}</Badge></div></div><Toast m={store.toast}/>
...
const v30ToolDetails:any={
  public_landing:{title:'Öffentliche /l/[slug] Seite',category:'QR & Loyalty',metrics:[['Landingpages','2'],['QR Slugs','2'],['Mobile','optimiert'],['Status','aktiv']],items:['/l/demo-cafe-morgenlicht','Review-Weiterleitung','Loyalty-Verknüpfung','Branding vorbereitet']},
  loyalty:{title:'Loyalty Programm',category:'QR & Loyalty',metrics:[['Programme','1'],['Punkte/Scan','10'],['Mitglieder','3'],['Status','aktiv']],items:['QR-Code-basiertes Punkteprogramm','Nachträglich mit QR-Kampagnen verknüpfbar','Endkunden-Erkennung über Token/Device/E-Mail','Kundenbereich-Konfiguration sichtbar']},
  loyalty_rewards:{title:'Rewards',category:'QR & Loyalty',metrics:[['Rewards','3'],['Einlösungen','1'],['Top Reward','Cappuccino'],['Status','aktiv']],items:['Gratis Cappuccino','10% Frühstücksrabatt','VIP Kuchenstück','Reward-Einlösung mit Mitarbeiterbestätigung']},
  loyalty_rules:{title:'Reward Regeln',category:'QR & Loyalty',metrics:[['Regeln','2'],['Bonus','2x'],['VIP Bonus','25'],['Status','aktiv']],items:['Punktebedarf je Reward','Reward-Typ: Rabatt / Gratisprodukt / VIP','Zeitfenster-Regeln','Segment-Regeln']},
  staff_codes:{title:'Mitarbeiter-Bestätigungscode',category:'QR & Loyalty',metrics:[['Codes','1'],['Demo Code','2468'],['Nutzungen','3'],['Status','aktiv']],items:['Code im Kundenbereich festlegbar','Reward-Einlösung wird erst nach Code bestätigt','Nutzungszähler vorhanden','Gültigkeit/Limit vorbereitet']},
  loyalty_segments:{title:'Loyalty Segmente',category:'QR & Loyalty',metrics:[['Segmente','4'],['VIP','1'],['Inaktiv','1'],['Reward-ready','2']],items:['VIP Kunden','Inaktive Endkunden','Reward-bereit','Review-Aktiv']},
  smart_loyalty:{title:'Smart Loyalty V2',category:'QR & Loyalty',metrics:[['Level','4'],['Multiplikator','1.5x'],['Rules','2'],['Actions','bereit']],items:['Basic / Silver / Gold / VIP','Punkte-Multiplikatoren','Smart Actions','Kampagnen-Verknüpfung']},
  reviews:{title:'Reviews',category:'Reviews',metrics:[['Reviews','3'],['Ø Rating','3.67'],['Google QR','94'],['Status','aktiv']],items:['QR Review Funnel','Interne Feedbacks','Google Weiterleitung','Review-Bonus für Loyalty']},
  review_intelligence:{title:'Review Intelligence',category:'Reviews',metrics:[['Sentiment','13'],['Positiv','1'],['Neutral','1'],['Negativ','1']],items:['Themenanalyse','Wartezeit erkannt','Sauberkeit erkannt','Antwortvorschläge und Ticket-Eskalation']},
  review_templates:{title:'Antwortvorlagen',category:'Reviews',metrics:[['Templates','3'],['Positiv','1'],['Neutral','1'],['Negativ','1']],items:['Positive Bewertung bedanken','Negative Bewertung entschärfen','Neutrale Bewertung beantworten','Kundenspezifische Templates vorbereitet']},
  smart_automation:{title:'Smart Automation',category:'Automation',metrics:[['Regeln','2'],['Runs','2'],['Trigger','QR/Review'],['Status','aktiv']],items:['Negatives Review → Ticket','Starke QR Kampagne → Upsell','Assistant-Hinweis erzeugen','Pipeline-Lead erzeugen']},
  marketing_automation:{title:'Marketing Automation',category:'Marketing',metrics:[['Kampagnen','2'],['Reaktivierung','1'],['Review Booster','1'],['Status','bereit']],items:['Inaktive Gäste zurückholen','Review Booster','Loyalty Reward verknüpfen','QR Kampagne verknüpfen']},
  ai_assistant:{title:'AI Business Assistant',category:'AI',metrics:[['Hinweise','3'],['Upsell','hoch'],['Risiko','niedrig'],['Status','aktiv']],items:['Hohe Upsell-Chance erkannt','Review-Thema Wartezeit prüfen','Paketnutzung sehr gut','Nächste Aktion empfehlen']},
  customer_health:{title:'Customer Health',category:'Analytics',metrics:[['Health','86'],['Revenue','82'],['Usage','91'],['QR','94']],items:['Warnungen aus Reviews','Chancen aus QR-Performance','Support Health','Loyalty Health']},
  customer_intelligence:{title:'Customer Intelligence',category:'Analytics',metrics:[['Risk','22'],['Upsell','84'],['Usage','91'],['Success','86']],items:['Risk Score','Upsell Score','Package Usage Score','Recommendations']},
  dynamic_billing:{title:'Dynamic Billing',category:'Billing',metrics:[['QR Scans','280'],['AI Messages','3'],['Runs','2'],['Addon','7 €']],items:['Usage-basierte Zusatzabrechnung','QR Scans','Automation Runs','AI Messages']},
  revenue_forecasting:{title:'Revenue Forecasting',category:'Analytics',metrics:[['Forecast','1.442 €'],['MRR','299 €'],['Pipeline','1.198 €'],['Confidence','82']],items:['Monatsforecast','Pipeline weighted value','Churn Risk Value','Revenue Drivers']},
  revenue_share:{title:'Revenue Share',category:'Billing',metrics:[['Share','15%'],['Gross','299 €'],['Share','45 €'],['Status','berechnet']],items:['Revenue Share Regeln','Partner Account vorbereitet','Stripe Connect vorbereitet','Share Events']},
  package_recommendations:{title:'Package Recommendations',category:'Billing',metrics:[['Empfehlungen','1'],['Uplift','499 €'],['Confidence','86'],['Priority','hoch']],items:['Premium Add-on empfehlen','QR Nutzung als Signal','Loyalty Nutzung als Signal','Upsell-Workflow']},
  package_matrix:{title:'Paket-Matrix',category:'Billing',metrics:[['Starter','5 Tools'],['Growth','10 Tools'],['Premium','28 Tools'],['Status','sync']],items:['Tool-Freigaben','Paketumfang','Kundenbereich-Sichtbarkeit','Admin-only Tools']},
  timeline_events:{title:'Timeline Events',category:'CRM',metrics:[['Events','5'],['QR','1'],['Loyalty','1'],['Automation','1']],items:['QR Kampagne erstellt','Reward eingelöst','Kritische Bewertung erkannt','Revenue Forecast aktualisiert']}
}
function V30ToolModule({view,store,cid,role}:any){
 const d=v30ToolDetails[view]||{title:view,category:'Tool',metrics:[],items:[]}
 return <><Head title={d.title} sub={`${d.category} · ${role==='admin'?'Adminbereich':'Kundenbereich'}`}/><div className="grid4">{d.metrics.map((m:any)=><Metric key={m[0]} label={m[0]} value={m[1]}/>)}</div><div className="grid2"><Card title="Funktionen">{d.items.map((x:string)=><div className="item" key={x}><b>{x}</b><Badge type="green">aktiv</Badge></div>)}</Card><Card title="Datenverknüpfung"><div className="item"><b>Kunde</b><span>{cname(store.data,cid)}</span></div><div className="item"><b>Paket</b><span>{cpkg(store.data,cid)}</span></div><div className="item"><b>Quelle</b><span>Demo + Supabase Sync</span></div><div className="sub">Dieses Modul ist im Demo-Tool sichtbar und über Paket-/Toolfreigaben einsortiert.</div></Card></div></>
}
