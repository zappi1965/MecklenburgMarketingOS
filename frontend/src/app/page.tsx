...
const seed:any={
 customers:[
  {id:ids.barber,name:'Barber Lounge Rostock',branch:'Friseur',email:'kontakt@barber.de',phone:'0381 123456',address:'Kröpeliner Str. 12',city:'Rostock',package_name:'Growth',is_demo:false},
  {id:ids.roof,name:'NordDach GmbH',branch:'Dachdecker',email:'kontakt@norddach.de',phone:'0385 987654',address:'Wismarsche Str. 88',city:'Schwerin',package_name:'Premium',is_demo:false},
  {id:ids.restaurant,name:'Alexas Inselblick',branch:'Restaurant',email:'kontakt@alexas.de',phone:'03991 123456',address:'Am See 4',city:'Waren',package_name:'Starter',is_demo:false}
 ],
 demo_customers:[
...
function PackageControl({store,cid,activeAdmin}:any){
 ...
 return <Card title="Paket & Toolfreigabe">{Object.keys(packageDefs).map(p=><div className="item" key={p}><div><b>{p}</b><div className="sub">{eur(pprice(p))}</div><FeatureList pkg={p}/></div><button className="btn secondary" onClick={()=>apply(p)}>{cpkg(store.data,cid)===p?'Aktiv':'Freischalten'}</button></div>)}</Card>
}
...
function MediaCenter({store,cid,activeAdmin}:any){const [type,setType]=useState<FileType>('media');return <><Head title="Media Center" sub="Dateien landen je nach Typ im passenden CRM-Bereich."/><select className="input" value={type} onChange={e=>setType(e.target.value as FileType)}><option value="invoices">Rechnung</option><option value="contracts">Vertrag</option><option value="media">Bilder / Medien</option><option value="documents">Dokument</option><option value="reports">Report</option></select><StorageUploader store={store} cid={cid} fileType={type} title="Datei hochladen" activeAdmin={activeAdmin}/><Card title="Gespeicherte Dateien"><FileList store={store} cid={cid}/></Card></>}
