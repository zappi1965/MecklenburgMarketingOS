...
function QRCodes({store,cid,setCid,role='admin',activeView='qr'}:any){
 ...
 function QrKpiCards(){return <><div className="grid4 qrKpiGrid"><Metric label="Aktive QR-Kampagnen" value={qrKpis.active}/><Metric label="QR-Scans" value={qrKpis.totalScans}/><Metric label="Punkte gesammelt" value={isStarterPackage?'–':qrKpis.points}/><Metric label="Prämien eingelöst" value={isStarterPackage?'–':qrKpis.rewards}/></div>{isStarterPackage&&<Card title="Starter-Paket aktiv"><div className="sub">Dieser Kunde sieht QR-Kampagnen, QR-Zielseite und Bewertungslogik. Punkte, Prämien, Bonusstufen, Kundengruppen und Automationen bleiben ausgeblendet, außer sie werden einzeln freigeschaltet.</div></Card>}</>}
 function QrSectionTabs(){return <Card title="QR Kampagnen verwalten"><div className="toolbarActions qrSectionTabs mobileTabRail">{allowedQrSections.map((s:any)=><button key={s.key} type="button" className={section===s.key?'btn':'btn secondary'} onClick={()=>setSection(s.key)}><b>{s.label}</b><span className="sub">{s.hint}</span></button>)}</div><div className="sub">{packageName==='Starter'?'Starter zeigt nur Kampagnen, QR-Zielseite und Bewertungen. Punkte, Prämien und Codes sind Growth/Premium oder einzeln freischaltbar.':'Punkte, Prämien, Regeln und Codes sind direkt hier gebündelt.'}</div></Card>}
...
function SalesWorkflowCenter({store,cid,setCid,setView,activeAdmin}:any){
 ...
 async function createPilotWorkflow(){
  ...
  await store.create('qr_campaigns',{customer_id:customer.id,title:'Review QR Kampagne',name:'Review QR Kampagne',slug,public_url:publicSlugUrl(slug),target_url:`/l/${slug}`,purpose:'review',status:'Aktiv',active:true,scans:0,conversions:0,created_at:new Date().toISOString()})
...
function V42QrCodePanel({cid}:any){
 ...
 return <Card title="QR Kampagne erstellen" action={<button className="btn secondary" onClick={create}>QR + Punkteprogramm erstellen</button>}>{url?<div className="qrExport"><V424QrImage value={url}/><div><b>{url}</b><p className="sub">QR wird serverseitig im MMOS-Backend erzeugt — keine Übermittlung an Drittanbieter.</p><div className="toolbarActions"><button className="btn secondary" onClick={()=>navigator.clipboard?.writeText(url)}>Link kopieren</button><button className="btn secondary" onClick={()=>window.open(url,'_blank')}>Slug öffnen</button><a className="btn secondary" href={qrServerUrl(url)} target="_blank" rel="noopener">QR als Bild öffnen</a></div></div></div>:<div className="sub">Noch kein QR erstellt. Klicke auf „QR + Punkteprogramm erstellen“.</div>}<input className="input" value={manualSlug} onChange={e=>setManualSlug(e.target.value)} placeholder="Optional: vorhandenen Slug manuell eingeben, z. B. kunde-bonusclub"/>{msg&&<div className="sub">{msg}</div>}</Card>
}
...
const v33ToolConfigs:any={
 qr:{title:'QR Kampagnen verwalten',category:'QR Kampagnen',resource:'qr_campaigns',fields:['title','purpose','points_per_scan','max_scans_per_member','daily_scan_limit_per_member','scan_cooldown_minutes','daily_point_limit_per_member','suspicion_score_threshold'],defaults:[],special:'qr'},
