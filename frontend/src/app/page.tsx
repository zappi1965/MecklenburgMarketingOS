...
function QRCodes({store,cid,setCid,role='admin'}:any){
 ...
 async function createLocalQr(seedData:any={}){
  const slug=seedData.slug||slugifyLocal(`${cname(store.data,customer)} ${f.title} ${Date.now()}`,'qr-kampagne')
  const public_url=seedData.public_url||publicSlugUrl(slug)
  const row={id:seedData.id||uid(),customer_id:customer,title:f.title||seedData.title||'QR Kampagne',name:f.title||seedData.name||'QR Kampagne',slug,public_url,target_url:`/l/${slug}`,...f,status:'Aktiv',active:true,scans:0,conversions:0,created_at:new Date().toISOString()}
 ...
 async function v34CreateQrCampaign(){
  try{
   ...
   store.notify?.(`QR/Loyalty Kampagne erstellt: ${r.public_url_path}`)
   setF({...f,title:''})
   setCid?.(customer)
  }catch(e:any){
   await createLocalQr()
...
function V42PublicLandingManager({store,cid}:any){
 ...
 const [form,setForm]=useState({title:`${customer?.name||'Kunde'} Bonusclub`,slug:slugifyLocal(customer?.name||'kunde'),headline:'Willkommen im Bonusclub',mode:'loyalty'})
 const campaigns=(store.data.qr_campaigns||[]).filter((q:any)=>q.customer_id===cid)
 async function create(){
  const slug=slugifyLocal(form.slug||form.title)
  await store.create('qr_campaigns',{customer_id:cid,title:form.title,name:form.title,slug,headline:form.headline,mode:form.mode,public_url:publicSlugUrl(slug),target_url:`/l/${slug}`,status:'Aktiv',active:true,created_at:new Date().toISOString()})
...
function V42AnalyticsBilling({store,cid}:any){
 ...
 return <><Head title="Analytics & Billing" sub="Live Kennzahlen · Usage · Billing" action={<button className="btn secondary" onClick={load}>Neu berechnen</button>}/><div className="grid4"><Metric label="QR Scans" value={a.qr_scans??'-'}/><Metric label="Reviews" value={a.reviews??'-'}/><Metric label="Health" value={a.health??'-'}/><Metric label="Forecast" value={eur(a.forecast??0)}/></div><div className="grid2"><Card title="Billing Berechnung"><div className="item"><b>Usage Total</b><span>{eur(data?.billing?.total??0)}</span></div><div className="item"><b>Revenue Share</b><span>{eur(a.revenue_share??0)}</span></div><div className="item"><b>Pipeline Value</b><span>{eur(a.pipeline_value??0)}</span></div>{msg&&<div className="sub">{msg}</div>}</Card><Card title="Analytics Signale"><div className="v40Bars">{[['QR',a.qr_scans||0],['Reviews',a.reviews||0],['Health',a.health||0],['Upsell',a.upsell||0]].map(([k,v]:any)=><div key={k}><b>{k}</b><div><span style={{width:`${Math.min(100,Number(v))}%`}}></span></div><em>{v}</em></div>)}</div></Card></div></>
}
...
function CRMInvoices({store,cid}:any){const rows=store.data.invoices.filter((i:any)=>i.customer_id===cid).sort((a:any,b:any)=>String(b.created_at).localeCompare(String(a.created_at)));return <Card title="Aktuelle Rechnungen"><div className="scrollBox">{rows.map((i:any)=><div className="item" key={i.id}><div><b>{i.invoice_number}</b><div className="sub">{i.service_type} · {eur(i.amount)} · {i.status}</div></div><div className="toolbarActions"><button className="btn secondary" onClick={()=>store.update('invoices',i.id,{status:i.status==='Bezahlt'?'Offen':'Bezahlt'})}>{i.status}</button><button className="btn secondary" onClick={()=>i.pdf_url||i.pdf_base64?openInvoicePdf(i):generateInvoicePdf(store,i)}>PDF öffnen</button><button className="btn secondary" onClick={()=>deleteInvoiceAndPdf(store,i)}>Löschen</button></div></div>)}</div><div className="sub">Hochgeladene Rechnungs-PDFs:</div><FileList store={store} cid={cid} type="invoices"/></Card>}
