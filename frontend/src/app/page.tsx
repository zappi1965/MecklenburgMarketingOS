...
function QRCodes({store,cid,setCid,role='admin',activeView='qr'}:any){
 ...
 const showLoyaltyKpis=!isStarterPackage||starterHasLoyaltyAddon
 ...
 async function createLocalQr(seedData:any={}){
  ...
  const row={id:seedData.id||uid(),customer_id:activeCustomer,title:f.title||seedData.title||'QR Kampagne',name:f.title||seedData.name||'QR Kampagne',slug,public_url:qrScanUrl(slug),target_url:`/q/${slug}`,purpose:isStarterPackage?'review':f.purpose,internal_email:f.internal_email,internal_from:f.internal_from,internal_to:f.internal_to,google_from:f.google_from,google_to:f.google_to,google_review_url:f.google_review_url,points_per_scan:isStarterPackage?0:Number(f.points_per_scan||1),metadata:{...(seedData.metadata||{}),purpose:isStarterPackage?'review':f.purpose,google_review_url:f.google_review_url,points_per_scan:isStarterPackage?0:Number(f.points_per_scan||1),max_scans_per_member:Number(f.max_scans_per_member||0),daily_scan_limit_per_member:Number(f.daily_scan_limit_per_member||0),scan_cooldown_minutes:Number(f.scan_cooldown_minutes||0),daily_point_limit_per_member:Number(f.daily_point_limit_per_member||0),suspicion_score_threshold:Number(f.suspicion_score_threshold||70),require_rescan_for_points:!isStarterPackage&&Number(f.points_per_scan||0)>0?f.require_rescan_for_points!==false:false,rotate_qr_after_scan:!isStarterPackage&&!!f.rotate_qr_after_scan,qr_scan_url:`/q/${slug}`,landing_url:`/l/${slug}`,final_slug_rules_source:'qr_campaigns'},status:'Aktiv',active:true,scans:0,conversions:0,created_at:new Date().toISOString()}
...
const v33ToolConfigs:any={
 ...
 loyalty_rewards:{title:'Prämien',category:'QR Kampagnen',resource:'loyalty_rewards',fields:['title','type','points','qr_campaign_id','staff_code_required','allow_multiple_redemptions','max_redemptions_per_member'],defaults:[]},
