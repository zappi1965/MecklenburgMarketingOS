...
export default function PublicLoyaltyPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    if (savedToken) {
      setLoading(true)
      v20GrowthClient.loyaltyJoinOrScan(slug, {
  ...
  async function collectPoints() {
    ...
    try {
      ...
      const r:any = await v20GrowthClient.loyaltyJoinOrScan(slug, {
        email,
        phone,
  ...
  async function sendFeedback() {
    ...
    try {
      ...
      await v20GrowthClient.reviewFeedback({
      ...
      {result?.member && (
        <section className="loyaltyCard">
          ...
          {reviewSent && rating && rating >= 4 && (
            <p className="success">Danke! Dein positives Feedback wurde gespeichert. Falls ein öffentlicher Bewertungslink hinterlegt ist, kann der Anbieter dich dorthin weiterleiten.</p>
          )}
