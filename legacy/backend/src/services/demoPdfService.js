...
async function createDemoInvoicePdf(invoice) {
  return bufferFromPdf((doc) => {
    ...
    doc.fontSize(12).text(invoice.customer_name || 'Interner Testkunde')
    doc.moveDown(1.5)
    ...
    doc.moveDown(2)
    doc.fontSize(10).text('Dies ist eine Testrechnung aus der isolierten MMOS Demo-Umgebung.')
  })
}
...
async function createDemoMonthlyReportPdf(payload) {
  return bufferFromPdf((doc) => {
    doc.fontSize(24).text('MecklenburgMarketingOS', { align: 'right' })
    doc.moveDown(1)
    doc.fontSize(26).text('Test Monatsreport')
    doc.moveDown(1)
    doc.fontSize(13).text(`Kunde: ${payload.customer_name || 'Interner Testkunde'}`)
