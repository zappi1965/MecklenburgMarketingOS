...
export default function Page() {
  ...
  async function load() {
    setMsg('Lade Dashboard...')
    const result = await customerPortalClient.overview().catch((e:any) => ({ ok:false, error:e.message }))
    setData(result)
    setMsg(result.ok ? 'Dashboard geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Reports", "Dokumente", "Einwilligungen", "Kampagnen"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Kunden-Dashboard V1</h1>
          <p className="muted">Kennzahlen, Reports und naechste Schritte aus dem Kundenportal.</p>
        </div>
