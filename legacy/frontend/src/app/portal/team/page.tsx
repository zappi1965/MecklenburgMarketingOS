...
export default function Page() {
  ...
  async function load() {
    ...
    setData(result)
    setMsg(result.ok ? 'Team geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Einwilligungen", "Dokumente", "Reports", "Kampagnen"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Team & Zugriffe V1</h1>
          <p className="muted">Basisansicht fuer spaetere Teamverwaltung und Rollen im Kundenportal.</p>
        </div>
