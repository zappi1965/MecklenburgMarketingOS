...
export default function Page() {
  ...
  async function load() {
    setMsg('Lade Kampagnen...')
    const result = await customerPortalClient.overview().catch((e:any) => ({ ok:false, error:e.message }))
    setData(result)
    setMsg(result.ok ? 'Kampagnen geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Kampagnen", "Einwilligungen", "Reports", "Dokumente"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Kampagnenfreigaben V1</h1>
          <p className="muted">Kundenseitige Uebersicht laufender Kampagnen und Entwuerfe.</p>
        </div>
