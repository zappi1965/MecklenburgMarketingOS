...
export default function Page() {
  ...
  async function load() {
    ...
    setData(result)
    setMsg(result.ok ? 'Reviews geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Dokumente", "Reports", "Kampagnen", "Einwilligungen"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Reviews & Feedback V1</h1>
          <p className="muted">Kundenseitige Sicht auf Feedback- und Reputation-Themen.</p>
        </div>
