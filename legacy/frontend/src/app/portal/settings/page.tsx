...
export default function Page() {
  ...
  async function load() {
    setMsg('Lade Einstellungen...')
    const result = await customerPortalClient.overview().catch((e:any) => ({ ok:false, error:e.message }))
    setData(result)
    setMsg(result.ok ? 'Einstellungen geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Dokumente", "Einwilligungen", "Reports", "Kampagnen"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Stammdaten & Einstellungen V1</h1>
          <p className="muted">Basisansicht fuer Unternehmensdaten, Datenschutz und Kontaktpunkte.</p>
        </div>
