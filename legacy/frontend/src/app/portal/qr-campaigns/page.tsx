...
export default function Page() {
  ...
  async function load() {
    setMsg('Lade QR-Kampagnen...')
    const result = await customerPortalClient.overview().catch((e:any) => ({ ok:false, error:e.message }))
    setData(result)
    setMsg(result.ok ? 'QR-Kampagnen geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Kampagnen", "Reports", "Dokumente", "Freigaben"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>QR-Kampagnen V1</h1>
          <p className="muted">Kundenseitige Sicht auf QR-/Slug-Kampagnen. Bearbeitung bleibt im Pilot intern.</p>
        </div>
