...
export default function Page() {
  ...
  async function load() {
    ...
    setData(result)
    setMsg(result.ok ? 'Billing geladen.' : result.error || 'Fehler')
  }

  useEffect(() => { void load() }, [])

  const m = data?.metrics || {}
  const metrics = ["Reports", "Dokumente", "Kampagnen", "Einwilligungen"]
    ...
    <main className="adminPage">
      <div className="pageHeader">
        <div>
          ...
          <h1>Paket & Billing V1</h1>
          <p className="muted">Basisansicht fuer Paket, Add-ons, Dokumente und Rechnungs-/Reportstatus.</p>
        </div>
