...
declare t text;
begin
  foreach t in array array['integrations','package_requests','customer_tool_access','contracts','reports','customer_files']
  loop
    execute format('drop policy if exists %I on public.%I', t || '_read', t);
    execute format('create policy %I on public.%I for select using (public.can_access_customer(customer_id))', t || '_read', t);
    execute format('drop policy if exists %I on public.%I', t || '_write', t);
...
-- =========================================================
-- MMOS SAFE LATE TABLES + RLS PATCH
-- Fix: create late tables before touching their policies.
-- =========================================================
...
for select using (customer_id is null or public.can_access_customer(customer_id));

drop policy if exists activity_logs_write on public.activity_logs;
create policy activity_logs_write on public.activity_logs
for all using (customer_id is null or public.can_access_customer(customer_id))
...
drop policy if exists notification_queue_read on public.notification_queue;
create policy notification_queue_read on public.notification_queue
for select using (customer_id is null or public.can_access_customer(customer_id));

drop policy if exists notification_queue_write on public.notification_queue;
create policy notification_queue_write on public.notification_queue
for all using (customer_id is null or public.can_access_customer(customer_id))
with check (customer_id is null or public.can_access_customer(customer_id));

drop policy if exists worker_jobs_read on public.worker_jobs;
create policy worker_jobs_read on public.worker_jobs
for select using (customer_id is null or public.can_access_customer(customer_id));

drop policy if exists worker_jobs_write on public.worker_jobs;
create policy worker_jobs_write on public.worker_jobs
for all using (customer_id is null or public.can_access_customer(customer_id))
with check (customer_id is null or public.can_access_customer(customer_id));
...
drop policy if exists customer_files_write on public.customer_files;
create policy customer_files_write on public.customer_files
for all using (public.can_access_customer(customer_id))
with check (public.can_access_customer(customer_id));

create index if not exists idx_activity_logs_customer_created on public.activity_logs(customer_id, created_at desc);
create index if not exists idx_workflow_runs_customer_created on public.workflow_runs(customer_id, created_at desc);
create index if not exists idx_notification_queue_status_created on public.notification_queue(status, created_at asc);
create index if not exists idx_worker_jobs_status_created on public.worker_jobs(status, created_at asc);
create index if not exists idx_customer_files_customer_created on public.customer_files(customer_id, created_at desc);



-- =========================================================
-- MMOS SAFE GENERIC POLICY ENSURE
-- =========================================================
do $$
declare t text;
begin
