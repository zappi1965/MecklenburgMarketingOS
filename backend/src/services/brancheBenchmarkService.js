...
function renderReportHtml({ branchLabel, period, comparison, usedFallback }) {
  const rows = comparison.map((c) => {
    ...
    return `<tr><td>${escapeHtml(c.label)}</td><td style="text-align:right">${escapeHtml(c.own_value ?? '–')}</td><td>${escapeHtml(peer)}</td><td>${escapeHtml(c.position || '–')}</td></tr>`
  }).join('')
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    <p class="sub">Branche: ${escapeHtml(branchLabel || '–')} · Zeitraum: ${escapeHtml(period.start)} bis ${escapeHtml(period.end)}</p>
