...
class EndcustomerReferralService {
  ...
  async creditOnFriendJoinScan({ customer_id, friend_member, referral_code }) {
    ...
    // V103.11.1: Öffentliche /r/[code]-Joins sollen Ende-zu-Ende funktionieren.
    // Wenn noch keine pending Referral-Zeile existiert, wird sie anhand des Codes
    // automatisch angelegt und anschließend im selben Join+Scan gutgeschrieben.
    if (!referral && byCode) {
      try {
        referral = await this.registerReferral({
          customer_id,
          referral_code: byCode,
          friend_email: friendEmail,
          source: 'public_referral_join_or_scan'
        })
      } catch (e) {
        return { skipped: true, reason: e.code || 'referral_auto_register_failed', error: e.message }
      }
    }
