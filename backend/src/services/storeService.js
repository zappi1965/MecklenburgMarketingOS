...
async function insertWithTimestampRetry(supabase,table,payload){
  ...
  const missingColumnFallback=stripMissingColumnFromPayload(payload,result.error)
  if(missingColumnFallback){
    result=await supabase.from(table).insert(missingColumnFallback).select('*').maybeSingle()
    if(!result.error)return result
  }
  if(String(table).toLowerCase()==='landing_page_settings'){
    let fallback=stripMissingColumnFromPayload(payload,result.error)||sanitizeLandingPageSettingsPayload(payload,'packages_only')
  ...
  if(isMissingColumnError(result.error,['updated_at','created_at'])){
    const fallback={...payload};delete fallback.updated_at;delete fallback.created_at
    result=await supabase.from(table).insert(fallback).select('*').maybeSingle()
  }
  return result
}
async function updateWithTimestampRetry(supabase,table,id,patch){
  ...
  if(!result.error)return result
  const missingColumnFallback=stripMissingColumnFromPayload(patch,result.error)
  if(missingColumnFallback){
    result=await supabase.from(table).update(missingColumnFallback).eq('id',id).select('*').maybeSingle()
    if(!result.error)return result
  }
  if(String(table).toLowerCase()==='landing_page_settings'){
    let fallback=stripMissingColumnFromPayload(patch,result.error)||sanitizeLandingPageSettingsPayload(patch,'packages_only')
  ...
  if(isMissingColumnError(result.error,['updated_at'])){
    const fallback={...patch};delete fallback.updated_at
    result=await supabase.from(table).update(fallback).eq('id',id).select('*').maybeSingle()
  }
  return result
}
