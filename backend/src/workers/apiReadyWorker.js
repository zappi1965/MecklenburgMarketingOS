...
async function mailJobs() {
  const { data } = await supabase.from('mail_jobs').select('*').eq('status', 'pending').limit(25)
  for (const job of data || []) {
    try {
      await mail.send({ to: job.to_email, subject: job.subject, html: job.html })
      await supabase.from('mail_jobs').update({ status: 'sent', sent_at: new Date().toISOString() }).eq('id', job.id)
    } catch (e) {
      await supabase.from('mail_jobs').update({ status: 'failed', last_error: e.message }).eq('id', job.id)
    }
  }
}

async function apiSyncJobs() {
  const { data } = await supabase.from('api_sync_jobs').select('*').eq('status', 'pending').limit(25)
  for (const job of data || []) {
    try {
      let result
      if (job.provider === 'google_business') result = await apiSync.syncGoogleBusiness(job.customer_id)
      if (job.provider === 'search_console') result = await apiSync.syncSearchConsole(job.customer_id, job.payload?.site_url)
      if (job.provider === 'analytics') result = await apiSync.syncAnalytics(job.customer_id, job.payload?.property_id)
      await supabase.from('api_sync_jobs').update({
        status: 'completed',
        processed_at: new Date().toISOString(),
        payload: { ...(job.payload || {}), result }
      }).eq('id', job.id)
    } catch (e) {
      await supabase.from('api_sync_jobs').update({ status: 'failed', last_error: e.message }).eq('id', job.id)
    }
  }
}
